    ...
    {   
		...
		if (poll(rd_poll,nrd,timeout))//有事件响应了
		{
			...
			if (respond_list[i]==0)//listenfd 有响应了,添加新的clientfd
			{
				...
				if (send(clientfd[nclientfd],"StuNo",5,0)<0)
               		error_exit("send");
				...
				switch (nstage[fd_no])
				{
					case 0:
					{
						if (recv(fd,&(pdia->StuNo),sizeof(int),0)<0)
							error_exit("recv");
						if (send(fd,"pid",3,0)<0)//send next stage msg
               				error_exit("send");
						break;
					}   
					case 1:
					...
					{/*
						...
						*/
						//int ret;
						//if (pollerr==POLLRDNORM)
							//ret=recv(fd,temp_buff,10,0);
